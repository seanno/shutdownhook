...
public class ZWay implements Closeable
{
	...
	public static class Config
	{
		...
		public Integer MaxUpdateIterations = 3;
		public Integer UpdateIntervalMilliseconds = 750;
	...
	public int getLevel(String deviceId) throws Exception {
		JsonObject json = getMetrics(deviceId);
		return(json.get("level").getAsInt());
	}
	
	public JsonObject getMetrics(String deviceId) throws Exception {
		...
		fetch(String.format("devices/%s/command/update", realDeviceId));

		long newUpdated = updated;
		int iter = 0;
		
		while (newUpdated == updated && iter++ < cfg.MaxUpdateIterations) {
			Thread.sleep(cfg.UpdateIntervalMilliseconds);
		...
		if (newUpdated == updated) {
			String msg = String.format("No update to %s after %d iterations; " +
									   "value may be stale",
									   deviceId, cfg.MaxUpdateIterations);
			log.warning(msg);
		}
